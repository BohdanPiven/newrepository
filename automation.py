    ...
    return render_template_string(
        ...
        <title>Panel Automatyzacji</title><style>
          *{margin:0;padding:0;box-sizing:border-box;}body{font-family:Arial,sans-serif;background:#f2f2f2;}
          ...
          <div class="container"><h1>Panel Automatyzacji</h1>
...
def automation_tiktok_plan():
    if "tiktok_open_id" not in session:
        flash("Musisz się połączyć z TikTok.", "error"); return redirect(url_for("automation.automation_tiktok"))
    ...
    if request.method == "POST":
        ...
        db.session.add(ScheduledPost(date=d, time=t,
                    topic=request.form["topic"], description=request.form["description"], user_id=uid))
...
def automation_tiktok_timeline():
    return render_template_string(
        """<!DOCTYPE html><html lang="pl"><head><meta charset="UTF-8">
        <title>Timeline TikTok</title><link href="https://cdn.jsdelivr.net/npm/fullcalendar@6.1.8/index.global.min.css" rel="stylesheet">
        <style>body{font-family:Arial,sans-serif;background:#f2f2f2;padding:20px}
          ...
          <script>
            document.addEventListener('DOMContentLoaded', () =>
              ...
              {initialView:'dayGridMonth',locale:'pl',events:'{{ url_for("automation.automation_tiktok_events") }}'}).render());
          </script></body></html>"""
    )
...
def automation_tiktok_video():
    ...
    if not f:
        flash("Nie wybrano pliku.", "error"); return redirect(url_for("automation.automation_tiktok_video"))
    ...
    try:
        ...
        if size == 0: raise ValueError("Rozmiar pliku nieznany (0 B).")
        ...
        init_resp = requests.post(
            ...
            headers={"Authorization": f"Bearer {session['tiktok_access_token']}",
                     "Accept":"application/json","X-Client-Id": TIKTOK_CLIENT_KEY or ""},
            json={"open_id": session["tiktok_open_id"],
                  "source_info":{"source":"FILE_UPLOAD","video_size":size,
                                 "chunk_size":chunk_size,"total_chunk_count":chunk_count}},
            timeout=15); current_app.logger.debug("INIT raw: %s", init_resp.text); init_resp.raise_for_status()
        ...
        # 2) UPLOAD – dopnij part_number=0 gdy jeden chunk
        if chunk_count == 1 and "part_number" not in upload_addr:
            sep = "&" if "?" in upload_addr else "?"
            upload_addr = f"{upload_addr}{sep}part_number=0"
        f.stream.seek(0)
        ...
        publish_endpoint = (
            ...
            if data.get("video_id") else
            "https://open.tiktokapis.com/v2/post/publish/inbox/video/upload/"
        )
        payload = {"video_id": video_id} if data.get("video_id") else {"publish_id": video_id}
        publish_resp = requests.post(publish_endpoint,
            headers={"Authorization": f"Bearer {session['tiktok_access_token']}",
                     "Accept":"application/json","X-Client-Id": TIKTOK_CLIENT_KEY or ""},
            json=payload, timeout=15)
        current_app.logger.debug("PUBLISH raw: %s", publish_resp.text); publish_resp.raise_for_status()
        status = publish_resp.json().get("data", {}).get("status", "PENDING")

    except requests.HTTPError as e:
        flash(f"Błąd TikTok API ({e.response.status_code}): {e.response.text[:400]}", "error")
        current_app.logger.error("[TikTok upload] %s | %s", e, e.response.text)
        return redirect(url_for("automation.automation_tiktok_video"))
    except Exception as ex:
        flash(f"Upload error: {ex}", "error"); current_app.logger.error("Upload crash: %s", ex)
        return redirect(url_for("automation.automation_tiktok_video"))
